...
### Optional handoff: local `/ultraplan`

If the Ship turn's implementation scope is non-trivial — multiple new files, new module, scaffolding-heavy refactor, or anything where you'd otherwise be tempted to "stop and plan first" — hand off to the **local `/ultraplan`** skill at the end of Turn 2 (after the spec is written) instead of executing inline in Turn 3.

**Use local `/ultraplan`, not `superpowers:writing-plans`.** The local skill produces a machine-dense numbered plan (exact paths, function signatures, test commands, context-cost footer) tuned to this workspace. The superpowers variant is generic and produces prose — disabled in favor of the local skill.

Trigger heuristics (any one suffices):
- Spec touches ≥3 distinct files
- A new module / new top-level directory is introduced
- Ship turn would otherwise need its own "let me first plan…" preamble
- User says "plan it out before shipping" / "give me the plan"

When triggered, Turn 3 becomes: invoke `/ultraplan` → user reads the dense plan → execute the plan as Turn 3's Ship. The three-turn budget is preserved; planning rides on top of Turn 2's spec.
